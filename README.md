...
| **3 · Agent Plane (ADK 2+)** | [`app/agent.py`](app/agent.py), [`app/callbacks/adk_callbacks.py`](app/callbacks/adk_callbacks.py), [`app/tools/agent_tools.py`](app/tools/agent_tools.py) | Hierarchical multi-agent system (**D2**) with environment-driven model tiering (**D6**, `GEMINI_PRO_MODEL` / `GEMINI_FLASH_MODEL` defaulting to `gemini-3.8-flash`): `root_agent` (zero tools — delegation only), `policy_agent` (read-only `search_policy`), `workweek_agent` (7 HCM tools), and `service_immediately_agent` (5 ITSM tools). |
